...
class CrcUsers extends PolymerElement {
  ...
  static get template() {
    return html`
      ...
      <crc-banner title="Users" description="test description"></crc-banner>

      <div class="container"> 
        <template is="dom-repeat" items="{{response.items}}"> 
          <crc-user-box firstname="[[item.data.firstname.iv]]" lastname="[[item.data.lastname.iv]]" email="[[item.data.email.iv]]" gender="[[item.data.gender.iv]]"> 
          </crc-user-box> 
        </template> 
      </div>
    `;
